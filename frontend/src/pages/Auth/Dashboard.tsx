  ...
  return (
    <DashboardLayout>
      <Box sx={{ px: { xs: 2, sm: 4 }, py: { xs: 2, sm: 3 } }}>
        <Typography variant="h5" fontWeight={700} mb={4} color="primary.main">
          Your CVs
        </Typography>
        ...
        {loading ? (
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
            {/* AddNewCvCard - Direct render, no wrapper */}
            <Box sx={{ width: { xs: '100%', sm: '48%', md: '31%', lg: '23%' } }}>
              <AddNewCvCard />
            </Box>

            {/* CV Cards - Direct render, no wrapper */}
            {cvs.map((cv) => (
              <Box 
                key={cv._id} 
